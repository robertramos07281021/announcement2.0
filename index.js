...
// --- CORS ---
const allowedOrigins = [process.env.MY_FRONTEND, 'http://localhost:5000'];
app.use(cors({
...
// --- GraphQL Schema ---
const resolvers = mergeResolvers([subscriptionsResolver, userResolvers, branchResolver, departmentResolver, announcementResolver]);
const typeDefs = mergeTypeDefs([UserTypeDefs, typesTypeDefs, subscriptionTypeDefs, branchTypeDefs, departmentTypeDefs, announcementTypeDefs]);
const schema = makeExecutableSchema({ typeDefs, resolvers });
...
app.use("/graphql", expressMiddleware(server, {
  context: async ({ req, res }) => {
    ...
    if (token) {
      try { user = await User.findById(jwt.verify(token, process.env.SECRET).id); } 
      catch {} 
    }
    return { req, res, user, pubsub, PUBSUB_EVENTS };
  },
}));
...
httpServer.listen(PORT, async () => {
  ...
  // Create default branch, department, admin user if missing
  const branch = await Branch.findOne({ name: "MAIN" }) || await new Branch({ name: "MAIN" }).save();
  const dept = await Department.findOne({ name: "ADMIN" }) || await new Department({ name: "ADMIN", branch: branch._id }).save();
  if (!await User.findOne({ username: "admin" })) {
    const hash = await bcrypt.hash(process.env.IT_PASS, 10);
    await new User({ username: "admin", password: hash, name: "admin", type: "ADMIN", department: dept._id }).save();
  }
});
